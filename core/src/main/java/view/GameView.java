...
// Import GameOverScreen to use its GameResult enum
...
public class GameView implements Screen {
    ...
    private boolean gameOver; // Indicates if game ending sequence has started
    ...
    public void show() {
        ...
        if (GdxWidth == 0 || GdxHeight == 0) {
            Gdx.app.error("GameView.show", "Gdx.graphics.getWidth() or getHeight() is 0.");
            return;
        }
        ...
        if (uiStage == null) {
            ...
            uiStage.getViewport().update((int)GdxWidth, (int)GdxHeight, true);
        }
        uiStage.clear();

        if (abilitySelectionStage == null) {
            abilitySelectionStage = new Stage(new ScreenViewport(), batch);
        } else {
            abilitySelectionStage.getViewport().update((int)GdxWidth, (int)GdxHeight, true);
        }
        ...
        if (skin == null) {
            Gdx.app.error("GameView.show", "Skin is null. Cannot create UI button.");
            skin = new Skin(Gdx.files.internal("skin/pixthulhu-ui.json"));
        }

        pauseButton = new TextButton("Pause", skin);
        pauseButton.setSize(100, 50);
        pauseButton.setPosition(GdxWidth - pauseButton.getWidth() - UI_SIDE_MARGIN,
            GdxHeight - pauseButton.getHeight() - UI_TOP_MARGIN);
        ...
        try {
            backgroundTexture = GameAssetManager.getGameAssetManager().getTexture(loadedBackgroundPath);
            if (backgroundTexture != null) {
                Gdx.app.log("GameView.show", "Successfully loaded specific background: " + loadedBackgroundPath);
            }
        } catch (Exception e) {
            Gdx.app.error("GameScreen", "Exception while trying to get '" + loadedBackgroundPath + "' from AssetManager.", e);
            backgroundTexture = null;
        }

        if (backgroundTexture == null) {
            Gdx.app.log("GameScreen", "Failed to load specific background: '" + loadedBackgroundPath + "'. Attempting fallback...");
            ...
            if (loadedBackgroundPath != null) {
                try { backgroundTexture = GameAssetManager.getGameAssetManager().getTexture(loadedBackgroundPath);
                    Gdx.app.log("GameScreen", "Using random background: " + loadedBackgroundPath);
                }
                catch (Exception e) {
                    Gdx.app.error("GameScreen", "Failed to load random background: " + loadedBackgroundPath, e);
                    backgroundTexture = null;
                }
            }
            if (backgroundTexture == null) {
                ...
                try { backgroundTexture = new Texture(Gdx.files.internal(loadedBackgroundPath));
                    Gdx.app.log("GameScreen", "Loaded hardcoded default_background.png directly.");
                }
                catch (Exception e) { Gdx.app.error("GameScreen", "CRITICAL: Failed to load default_background.png.", e); loadedBackgroundPath = null;}
            }
        }
        ...
        } else if (enemyController == null) { return; }
    ...
    private void togglePause() {
        if (currentGameState == GamePlayState.PLAYING) {
            ...
            Gdx.app.log("GameView", "Game Paused by user. Switching to Pause Menu.");
    ...
    private void enterAbilitySelection() {
        if (currentGameState != GamePlayState.ABILITY_SELECTION) {
            ...
            pauseStartTimeNanos = TimeUtils.nanoTime();
            Gdx.app.log("GameView", "Entering Ability Selection. Game time paused.");
    ...
    private void prepareAbilitySelectionUI() {
        ...
        for (final Ability ability : offeredAbilities) {
            ...
            abilityButton.getLabel().setAlignment(Align.center);
            abilityButton.addListener(new ClickListener() {
                @Override public void clicked(InputEvent event, float x, float y) {
                    applyAbility(ability);
                    exitAbilitySelection();
                }
            });
    ...
    private void applyAbility(Ability ability) {
        Gdx.app.log("GameView", "Applying ability: " + ability.getDisplayName());
    ...
    private void exitAbilitySelection() {
        if (currentGameState == GamePlayState.ABILITY_SELECTION) {
            ...
            if(player != null) player.resetLevelUpFlag();
            abilitySelectionStage.clear();
            Gdx.app.log("GameView", "Exiting Ability Selection. Game Resumed.");
            Gdx.input.setInputProcessor(inputMultiplexer);
        }
    }

    public void triggerGiveUp() {
        Gdx.app.log("GameView", "triggerGiveUp called.");
        if (!gameOver) {
    ...
    private void endGameAndShowScreen(GameResult result) {
        ...
        Gdx.app.postRunnable(() -> {
            if (Main.getMain().getScreen() instanceof GameView || Main.getMain().getScreen() == null) {
                ...
                Gdx.app.log("GameView", "endGameAndShowScreen: Not current screen, GameOverScreen not set by GameView.");
            }
        });
    }
    ...
    public void render(float delta) {
        if (currentGameState == GamePlayState.USER_PAUSED && Main.getMain().getScreen() != this) {
            return;
        }
        ...
        if (playerIdleAnim != null) {
            ...
            if (playerCurrentFrame != null) {
                ...
                batch.draw(playerCurrentFrame, worldPlayerPosition.x - playerScaledWidth / 2f,
    ...
    public void resize(int width, int height) {
        ...
        if (uiStage != null) {
            ...
            if (pauseButton != null) {
                pauseButton.setPosition(width - pauseButton.getWidth() - UI_SIDE_MARGIN,
                    height - pauseButton.getHeight() - UI_TOP_MARGIN);
            }
        }
        if (abilitySelectionStage != null) {
            abilitySelectionStage.getViewport().update(width, height, true);
            if (abilityTable != null && abilityTable.getParent().equals(abilitySelectionStage)) {
    ...
    public void pause() {
        if (!gameOver && currentGameState == GamePlayState.PLAYING) {
            currentGameState = GamePlayState.USER_PAUSED;
            pauseStartTimeNanos = TimeUtils.nanoTime();
            Gdx.app.log("GameView", "GameView.pause() (OS pause). GameState set to USER_PAUSED.");
        } else {
            Gdx.app.log("GameView", "GameView.pause() (OS pause). No action, current state: " + currentGameState);
        }
    }

    @Override
    public void resume() {
        Gdx.app.log("GameView", "GameView.resume() (OS resume). Current GameState: " + currentGameState);
    ...
    @Override
    public void hide() {
        Gdx.app.log("GameView", "hide() called. Current GameState: " + currentGameState);
        if (currentGameState == GamePlayState.PLAYING && !gameOver && pauseStartTimeNanos == 0) {
            currentGameState = GamePlayState.USER_PAUSED;
            pauseStartTimeNanos = TimeUtils.nanoTime();
            Gdx.app.log("GameView.hide", "Game was PLAYING, setting to USER_PAUSED and recording pauseStartTimeNanos.");
        }
    }
    ...
    public void dispose() {
        ...
        if (backgroundTexture != null) {
            ...
            if (GameAssetManager.getGameAssetManager() != null &&
                ...
                GameAssetManager.getGameAssetManager().getAssetManager().isLoaded(loadedBackgroundPath)) {
    ...
    private class GameInputProcessor implements com.badlogic.gdx.InputProcessor {
        ...
        public boolean keyDown(int keycode) {
            ...
            if (pauseKey != null && keycode == pauseKey) {
                if (currentGameState == GamePlayState.PLAYING) togglePause();
                return true;
            }
            ...
            if (currentGameState == GamePlayState.PLAYING) {
                if (keycode == Input.Keys.NUM_1) {
                    long timeToReduceNanos = 60 * 1_000_000_000L;
                    gameStartTime += timeToReduceNanos;
                    if (TimeUtils.nanoTime() < gameStartTime) {
                        gameStartTime = TimeUtils.nanoTime();
                    }
                    gameElapsedTimeSeconds = (TimeUtils.nanoTime() - gameStartTime) / 1_000_000_000.0f;
                    Gdx.app.log("CHEAT", "Time reduced by 1 minute. New elapsed: " + gameElapsedTimeSeconds);
                    return true;
                } else if (keycode == Input.Keys.NUM_2 && player != null) {
                    player.addXp(player.getXpToNextLevel() - player.getXp() + 1);
                    Gdx.app.log("CHEAT", "Player leveled up by cheat. New level: " + player.getLevel());
                    // Level up flag will be set in player, triggering ability selection in next render if conditions met
                    return true;
                } else if (keycode == Input.Keys.NUM_3 && player != null) {
                    player.increaseMaxHealth(1);
                    Gdx.app.log("CHEAT", "Player HP increased by cheat. Current: " + player.getCurrentHp() + "/" + player.getMaxHp());
                    return true;
                } else if (keycode == Input.Keys.NUM_4 && enemyController != null) {
                    enemyController.spawnBossNow();
                    return true;
                } else if (keycode == Input.Keys.NUM_5 && enemyController != null) {
                    enemyController.killRandomEnemies(10);
                    return true;
                }
            }
            ...
            if (currentGameState == GamePlayState.PLAYING) { // Only allow game actions if playing
            ...
            if (!gun.isReloading() && gun.getCurrentAmmo() > 0 && shootTimer >= gun.getGunData().getFire_rate()) {
                ...
                for (int i = 0; i < gun.getCurrentProjectiles(); i++) {
                    ...
                    if (gun.getCurrentProjectiles() > 1) {
                        float baseSpreadAngle = 5f;
                        float totalSpread = baseSpreadAngle * (gun.getCurrentProjectiles() -1) ;
